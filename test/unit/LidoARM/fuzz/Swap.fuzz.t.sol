...
contract Unit_Fuzz_LidoARM_Swap_Test is Unit_LidoARM_Shared_Test {
    ...
    function testFuzz_SwapExactTokensForTokens_Steth_To_Weth_BuyPrice(uint128 fuzzedBuyPrice) public {
        ...
        deal(address(weth), address(lidoARM), wethSeed);
        deal(address(steth), alice, amountIn);
        ...
        uint256 buyPriceFuzzed;
        {
            ...
            vm.prank(governor);
            lidoARM.setPrices(address(steth), buyPriceFuzzed, sellPriceArg, type(uint128).max, type(uint128).max);
        }
        ...
        assertApproxEqAbs(lidoARM.feesAccrued(), expectedFee, 2);
        assertEq(buyLiquidityRemaining(steth), buyLiquidityBefore - expectedAmountOut);
        assertEq(sellLiquidityRemaining(steth), sellLiquidityBefore);
        assertEq(weth.balanceOf(alice), expectedAmountOut);
        assertEq(steth.balanceOf(alice), 0);
        ...
        assertEq(weth.balanceOf(address(lidoARM)), wethSeed - expectedAmountOut);
        assertEq(steth.balanceOf(address(lidoARM)), amountIn);
        assertEq(amounts.length, 2);
        assertEq(amounts[0], amountIn);
        assertEq(amounts[1], expectedAmountOut);
        ...
        assertGt(lidoARM.totalAssets(), totalAssetsBefore);
        assertApproxEqAbs(lidoARM.totalAssets(), totalAssetsBefore + expectedTotalAssetsIncrease - expectedFee, 2);
    }
    ...
    function testFuzz_SwapExactTokensForTokens_Weth_To_Steth_Amount(uint128 wethAmount) public {
        ...
        deal(address(steth), address(lidoARM), armStethSeed);
    ...
    function testFuzz_SwapExactTokensForTokens_Weth_To_Steth_SellPrice(uint128 fuzzedSellPrice) public {
        ...
        deal(address(steth), address(lidoARM), stethSeed);
        deal(address(weth), alice, amountIn);
        ...
        assertEq(weth.balanceOf(address(lidoARM)), armWethBefore + amountIn);
        assertEq(steth.balanceOf(address(lidoARM)), stethSeed - expectedAmountOut);
        assertEq(amounts.length, 2);
        assertEq(amounts[0], amountIn);
        assertEq(amounts[1], expectedAmountOut);
        assertGt(lidoARM.totalAssets(), totalAssetsBefore);
    ...
    function testFuzz_SwapTokensForExactTokens_Steth_To_Weth_BuyPrice(uint128 fuzzedBuyPrice) public {
        ...
        assertEq(weth.balanceOf(address(lidoARM)), wethSeed - amountOut);
        assertEq(steth.balanceOf(address(lidoARM)), expectedAmountIn);
        assertEq(amounts.length, 2);
        assertEq(amounts[0], expectedAmountIn);
        assertEq(amounts[1], amountOut);
        // fee = gain * feeRate / FEE_SCALE algebraically, so with feeRate = 20% < 100% the gain
    ...
    function testFuzz_SwapTokensForExactTokens_Weth_To_Steth_SellPrice(uint128 fuzzedSellPrice) public {
        ...
        assertEq(weth.balanceOf(address(lidoARM)), armWethBefore + expectedAmountIn);
        assertEq(steth.balanceOf(address(lidoARM)), stethSeed - amountOut);
        assertEq(amounts.length, 2);
        assertEq(amounts[0], expectedAmountIn);
        assertEq(amounts[1], amountOut);
        assertGt(lidoARM.totalAssets(), totalAssetsBefore);
