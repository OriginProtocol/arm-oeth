...
contract Unit_Fuzz_LidoARM_ClaimRedeem_Test is Unit_LidoARM_Shared_Test {
    ...
    function testFuzz_ClaimRedeem_Shares(uint128 fuzzedShares) public {
        ...
        uint256 reservedBefore = lidoARM.reservedWithdrawLiquidity();
        uint256 claimedSharesBefore = lidoARM.withdrawsClaimedShares();

        // Expect events
        vm.expectEmit({emitter: address(lidoARM)});
        emit IERC20.Transfer(address(lidoARM), address(0), shares);
        vm.expectEmit({emitter: address(weth)});
        emit IERC20.Transfer(address(lidoARM), alice, requestAssets);
        vm.expectEmit({emitter: address(lidoARM)});
        emit AbstractARM.RedeemClaimed(alice, requestId, requestAssets);

        // When
        vm.prank(alice);
        uint256 assets = lidoARM.claimRedeem(requestId);
        ...
        assertEq(lidoARM.totalSupply(), supplyBefore - shares, "totalSupply");
        assertEq(lidoARM.totalAssets(), supplyBefore - shares, "totalAssets matches at 1:1");
    ...
    function testFuzz_ClaimRedeem_GainAfterRequest(uint128 fuzzedYield) public {
        ...
        assertGt(lidoARM.convertToAssets(50 ether), requestAssets, "claim-time value above request");

        // Expect events
        vm.expectEmit({emitter: address(lidoARM)});
        emit IERC20.Transfer(address(lidoARM), address(0), 50 ether);
        vm.expectEmit({emitter: address(weth)});
        emit IERC20.Transfer(address(lidoARM), alice, requestAssets);
        vm.expectEmit({emitter: address(lidoARM)});
        emit AbstractARM.RedeemClaimed(alice, requestId, requestAssets);
    ...
    function testFuzz_ClaimRedeem_LossAfterRequest(uint128 fuzzedLoss) public {
        ...
        assertEq(lidoARM.convertToAssets(100 ether), expectedPayout, "convertToAssets matches expected");

        // Expect events (loss path: payout < requestAssets)
        vm.expectEmit({emitter: address(lidoARM)});
        emit IERC20.Transfer(address(lidoARM), address(0), 100 ether);
        vm.expectEmit({emitter: address(weth)});
        emit IERC20.Transfer(address(lidoARM), alice, expectedPayout);
        vm.expectEmit({emitter: address(lidoARM)});
        ...
        assertEq(lidoARM.reservedWithdrawLiquidity(), 0, "reserved released in full");
        assertEq(lidoARM.withdrawsClaimedShares(), 100 ether, "withdrawsClaimedShares");
