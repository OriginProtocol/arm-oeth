...
contract Unit_Concrete_OriginARM_Migration_Test is Unit_Shared_Test {
    function test_MigrateFeesAccrued_ClearsLegacyBitsAndPreservesFee() public {
        ...
        assertEq(
            uint256(feeStorageSlot(address(originARM))) >> 16,
            legacyValue,
            "legacy bits should be visible before migration"
        );
        ...
        assertEq(uint256(feeStorageSlot(address(originARM))) >> 16, 0, "storage should be reset");
