...
contract Unit_Concrete_OriginARM_AvailableLiquidity_Test_ is Unit_Shared_Test {
    ...
    function test_AvailableLiquidity_AfterDeposit() public deposit(alice, DEFAULT_AMOUNT) {
        (uint256 balance0, uint256 balance1) = _getReserves();
    ...
    function test_RevertWhen_GetReserves_Because_UnsupportedBaseAsset() public {
        vm.expectRevert("ARM: unsupported asset");
        originARM.getReserves(address(weth));
    }

    function _getReserves() internal view returns (uint256 reserve0, uint256 reserve1) {
        (reserve0, reserve1) = originARM.getReserves(address(oeth));
    }
}
