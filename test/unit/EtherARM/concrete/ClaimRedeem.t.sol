...
contract Unit_EtherARM_ClaimRedeem_Test is Unit_EtherARM_Shared_Test {
    ...
    function test_ClaimRedeem_Default() public {
        ...
        assertEq(etherARM.balanceOf(address(etherARM)), expectedShares);
        assertEq(etherARM.totalAssets(), expectedAssets + 1e12);
        assertEq(etherARM.totalSupply(), expectedShares + 1e12);
        assertEq(etherARM.reservedWithdrawLiquidity(), expectedAssets);
        assertEq(etherARM.withdrawsClaimedShares(), 0);

        // Expect
        vm.expectEmit({emitter: address(etherARM)});
        emit IERC20.Transfer(address(etherARM), address(0), expectedShares);
        vm.expectEmit({emitter: address(weth)});
        emit IERC20.Transfer(address(etherARM), alice, expectedAssets);
        vm.expectEmit({emitter: address(etherARM)});
        ...
        assertEq(etherARM.balanceOf(address(etherARM)), 0, "escrow");
        assertEq(etherARM.totalAssets(), 1e12, "totalAssets");
        assertEq(etherARM.totalSupply(), 1e12, "totalSupply");
        assertEq(etherARM.reservedWithdrawLiquidity(), 0, "reserved");
        assertEq(etherARM.withdrawsClaimedShares(), expectedShares, "claimed");
    }
    ...
    function test_ClaimRedeem_LossAfterRequest() public {
        ...
        assertEq(etherARM.balanceOf(address(etherARM)), expectedShares);
        assertEq(etherARM.totalAssets(), expectedTotalAssets);
        assertEq(etherARM.totalSupply(), expectedShares + 1e12);
        assertEq(etherARM.reservedWithdrawLiquidity(), 100 ether);
        assertEq(etherARM.withdrawsClaimedShares(), 0);
        ...
        emit IERC20.Transfer(address(etherARM), address(0), expectedShares);
        vm.expectEmit({emitter: address(weth)});
        emit IERC20.Transfer(address(etherARM), alice, expectedAliceAssets);
        vm.expectEmit({emitter: address(etherARM)});
    ...
    function test_ClaimRedeem_LossAfterBothQueued() public {
        ...
        uint256 expectedShares = 100 ether; // Shares are not affected by the loss
        assertEq(etherARM.balanceOf(address(etherARM)), expectedShares * 2);
        assertEq(etherARM.totalAssets(), expectedTotalAssets);
        assertEq(etherARM.totalSupply(), expectedShares * 2 + 1e12);
        assertEq(etherARM.reservedWithdrawLiquidity(), 200 ether);
        assertEq(etherARM.withdrawsClaimedShares(), 0);
        ...
        // When Bobby claims
        vm.prank(bobby);
        etherARM.claimRedeem(1);

        // Then
        assertEq(weth.balanceOf(alice), expectedAliceAssets, "alice weth");
        assertEq(weth.balanceOf(bobby), expectedBobbyAssets, "bobby weth");
        assertEq(etherARM.balanceOf(alice), 0, "alice shares");
        assertEq(etherARM.balanceOf(bobby), 0, "bobby shares");
        assertEq(etherARM.balanceOf(address(etherARM)), 0, "escrow");
        // We have the minimum returned by totalAssets, but in theory it should be 0.8e12.
        assertEq(etherARM.totalAssets(), 1e12, "totalAssets");
        assertEq(etherARM.totalSupply(), 1e12, "totalSupply");
        assertEq(etherARM.reservedWithdrawLiquidity(), 0, "reserved");
        assertEq(etherARM.withdrawsClaimedShares(), expectedShares * 2, "claimed");
    }
    ...
    function test_ClaimRedeem_LossBeforeBothRequests() public {
        ...
        assertEq(etherARM.totalAssets(), 160 ether + 8e11, "totalAssets after loss");
        assertEq(etherARM.totalSupply(), 200 ether + 1e12, "totalSupply after loss");
        assertEq(etherARM.convertToAssets(1 ether), 0.8 ether, "share price after loss");
        ...
        // Alice claims.
        vm.prank(alice);
        etherARM.claimRedeem(aliceRequestId);
        ...
        assertEq(etherARM.totalAssets(), 120 ether + 8e11, "totalAssets after alice claim");
        assertEq(etherARM.totalSupply(), 150 ether + 1e12, "totalSupply after alice claim");
        assertEq(etherARM.convertToAssets(1 ether), 0.8 ether, "share price after alice claim");
        ...
        // Bobby claims.
        vm.prank(bobby);
        etherARM.claimRedeem(bobbyRequestId);
        ...
        assertEq(etherARM.balanceOf(bobby), 75 ether, "bobby remaining shares");
    ...
    function test_ClaimRedeem_RecoveryBetweenClaims() public {
        ...
        assertEq(etherARM.reservedWithdrawLiquidity(), 50 ether, "reserved at pre-loss price");
        ...
        deal(address(weth), address(etherARM), weth.balanceOf(address(etherARM)) + recoveryAmount);
        assertEq(etherARM.totalAssets(), 135 ether + 9e11, "totalAssets after recovery");
        assertEq(etherARM.convertToAssets(1 ether), 0.9 ether, "share price after recovery");
    ...
    function test_ClaimRedeem_GainAfterRequest() public {
        ...
        assertEq(etherARM.totalAssets(), 220 ether + 1e12, "totalAssets with yield");
        assertEq(etherARM.totalSupply(), 200 ether + 1e12, "totalSupply unchanged");
        ...
        vm.prank(alice);
        uint256 aliceClaimed = etherARM.claimRedeem(aliceId);
        ...
        uint256 bobShareValue = etherARM.previewRedeem(100 ether);
    ...
    function test_ClaimRedeem_TwoLossesSeparatedByClaim() public {
        ...
        assertEq(etherARM.convertToAssets(1 ether), 0.8 ether, "price after loss #1");
        ...
        assertEq(etherARM.convertToAssets(1 ether), 0.8 ether, "price still 0.8 after alice claim");
        assertEq(etherARM.totalAssets(), 120 ether + 8e11, "totalAssets after alice claim");
        assertEq(etherARM.totalSupply(), 150 ether + 1e12, "totalSupply after alice claim");
        ...
        assertEq(etherARM.totalAssets(), 96 ether + 64e10, "totalAssets after loss #2");
        // New price: 0.8 * 0.8 = 0.64
        assertEq(etherARM.convertToAssets(1 ether), 0.64 ether, "price after loss #2");
        ...
        skip(CLAIM_DELAY);
        vm.prank(bobby);
        uint256 bobClaimed = etherARM.claimRedeem(bobId);
    ...
    function test_ClaimRedeem_WithActiveMarket() public {
        ...
        etherARM.allocate();
        assertEq(weth.balanceOf(address(etherARM)), 0, "ARM drained to market");
        assertGt(mockERC4626Market.balanceOf(address(etherARM)), 0, "ARM holds market shares");
        ...
        assertEq(weth.balanceOf(address(etherARM)), 0, "ARM left with no WETH");
    }
    ...
    function test_ClaimRedeem_FullReservationReleasedOnLoss() public {
        ...
        assertEq(etherARM.reservedWithdrawLiquidity(), 100 ether, "reservation at request time");
        ...
        assertEq(etherARM.reservedWithdrawLiquidity(), 0, "full reservation released");
        ...
        assertEq(weth.balanceOf(address(etherARM)), 80 ether + 8e11, "balance after claim");
        assertEq(etherARM.totalSupply(), 100 ether + 1e12, "supply after burn");
        assertEq(etherARM.convertToAssets(1 ether), 0.8 ether, "price still 0.8 for remaining LPs");
    }
    ...
    function test_ClaimRedeem_RevertWhen_InsufficientLiquidity() public {
        ...
        assertEq(etherARM.reservedWithdrawLiquidity(), 200 ether, "reserved = 200");
        ...
        deal(address(steth), address(etherARM), 150 ether);

        // Sanity: state is "balance-poor, totalAssets-rich"
        assertEq(weth.balanceOf(address(etherARM)), 50 ether + 1e12, "balance WETH = 50");
        assertEq(etherARM.totalAssets(), 200 ether + 1e12, "totalAssets unchanged");
        assertEq(etherARM.totalSupply(), 200 ether + 1e12, "totalSupply unchanged");
