...
contract Unit_Concrete_EtherARM_SwapExactTokensForTokens_Test is Unit_EtherARM_Shared_Test {
    ...
    function test_SwapExactTokensForTokens_Steth_To_Weth_Router() public {
        ...
        uint256[] memory amounts =
            etherARM.swapExactTokensForTokens(amountIn, expectedAmountOut, path, alice, block.timestamp);
    ...
    function test_SwapExactTokensForTokens_Steth_To_Weth_UseMarket() public {
        ...
        uint256 expectedMarketWithdrawal = expectedAmountOut - weth.balanceOf(address(etherARM));
        uint256 buyLiquidityBefore = buyLiquidityRemaining(steth);
        uint256 sellLiquidityBefore = sellLiquidityRemaining(steth);
        uint256 armStethBefore = steth.balanceOf(address(etherARM));
    ...
    function test_SwapExactTokensForTokens_Wsteth_To_Weth_Default() public {
        ...
        // When
        vm.prank(alice);
        uint256[] memory amounts = etherARM.swapExactTokensForTokens(wsteth, weth, amountIn, expectedAmountOut, alice);
        ...
        assertEq(weth.balanceOf(address(etherARM)), armWethBefore - expectedAmountOut);
        assertEq(wsteth.balanceOf(address(etherARM)), armWstethBefore + amountIn);
        assertEq(amounts[0], amountIn);
        assertEq(amounts[1], expectedAmountOut);
        assertGt(etherARM.totalAssets(), totalAssetsBefore);
        // Note: Temporary 1 wei tolerance while the fee rounding issue is being fixed.
    ...
    function test_SwapExactTokensForTokens_Weth_To_Wsteth_Default() public {
        ...
        // When
        vm.prank(alice);
        uint256[] memory amounts = etherARM.swapExactTokensForTokens(weth, wsteth, amountIn, expectedAmountOut, alice);
        ...
        assertEq(weth.balanceOf(address(etherARM)), armWethBefore + amountIn);
        assertEq(wsteth.balanceOf(address(etherARM)), armWstethBefore - expectedAmountOut);
        assertEq(amounts[0], amountIn);
        assertEq(amounts[1], expectedAmountOut);
        assertGt(etherARM.totalAssets(), totalAssetsBefore);
        assertEq(etherARM.totalAssets(), totalAssetsBefore + expectedTotalAssetsIncrease);
    ...
    function test_SwapExactTokensForTokens_RevertWhen_InvalidSwapAssets() public {
        ...
        etherARM.swapExactTokensForTokens(steth, steth, 1 ether, 1 ether, alice);

        // Same token for both sides of the swap, even if it's liquidity asset
        vm.expectRevert(AbstractARM.InvalidSwapAssets.selector);
        etherARM.swapExactTokensForTokens(weth, weth, 1 ether, 1 ether, alice);

        // Both tokens are base assets supported by the ARM
        vm.expectRevert(AbstractARM.InvalidSwapAssets.selector);
        etherARM.swapExactTokensForTokens(steth, wsteth, 1 ether, 1 ether, alice);

        // Unsupported token as liquidity asset
        vm.expectRevert(AbstractARM.InvalidSwapAssets.selector);
        etherARM.swapExactTokensForTokens(steth, IERC20(address(0x1234)), 1 ether, 1 ether, alice);

        // Unsupported token as base asset
        vm.expectRevert(AbstractARM.InvalidSwapAssets.selector);
        etherARM.swapExactTokensForTokens(weth, IERC20(address(0x1234)), 1 ether, 1 ether, alice);
    }
    ...
    function test_SwapExactTokensForTokens_RevertWhen_InsufficientLiquidity() public {
        ...
        etherARM.swapExactTokensForTokens(steth, weth, amountIn, 0, alice);
        ...
        vm.expectRevert(AbstractARM.InsufficientLiquidity.selector);
        etherARM.swapExactTokensForTokens(steth, weth, 10 ether, 0, alice);

        // Not enough buy liquidity at this price
        vm.expectRevert(AbstractARM.InsufficientLiquidity.selector);
        etherARM.swapExactTokensForTokens(weth, steth, 10 ether, 0, alice);
        ...
        vm.expectRevert(AbstractARM.InsufficientLiquidity.selector);
        etherARM.swapExactTokensForTokens(weth, steth, 7 ether, 0, alice);
    }
    ...
    function test_SwapExactTokensForTokens_RevertWhen_InsuffisantOutputAmount() public {
        ...
        etherARM.swapExactTokensForTokens(steth, weth, amountIn, amountOutMin, alice);
        ...
        vm.prank(alice);
        vm.expectRevert(AbstractARM.InsufficientOutputAmount.selector);
        etherARM.swapExactTokensForTokens(amountIn, amountOutMin, path, alice, block.timestamp);
    }
    ...
    function test_SwapExactTokensForTokens_RevertWhen_InvalidPathLength() public {
        ...
        etherARM.swapExactTokensForTokens(0, 0, path, alice, 0);
    }
    ...
    function test_SwapExactTokensForTokens_RevertWhen_DeadlineExpired() public {
        ...
        etherARM.swapExactTokensForTokens(0, 0, path, alice, block.timestamp - 1);
    }
    ...
    function test_SwapExactTokensForTokens_RevertWhen_Paused() public {
        ...
        etherARM.swapExactTokensForTokens(steth, weth, 1 ether, 0, alice);

        vm.expectRevert(AbstractARM.ContractPaused.selector);
        etherARM.swapExactTokensForTokens(1 ether, 0, new address[](2), alice, block.timestamp);
    }
}
