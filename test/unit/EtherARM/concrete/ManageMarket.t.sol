...
contract Unit_EtherARM_ManageMarket_Test is Unit_EtherARM_Shared_Test {
    ...
    function test_AddMarkets_Single() public {
        assertEq(etherARM.supportedMarkets(address(mockERC4626Market)), false, "supported pre");
        ...
        assertEq(etherARM.supportedMarkets(address(mockERC4626Market)), true, "supported post");
    }
    ...
    function test_AddMarkets_Multiple() public {
        ...
        assertEq(etherARM.supportedMarkets(markets[0]), true, "first supported");
        assertEq(etherARM.supportedMarkets(markets[1]), true, "second supported");
    }

    function test_AddMarkets_RevertWhen_NotOwner() public {
        vm.prank(alice);
        vm.expectRevert(Ownable.OnlyOwner.selector);
        etherARM.addMarkets(new address[](0));
    }

    function test_AddMarkets_RevertWhen_AddressZero() public {
        vm.prank(governor);
        vm.expectRevert(AbstractARM.InvalidMarket.selector);
        etherARM.addMarkets(new address[](1));
    }
    ...
    function test_RemoveMarket_Default() public {
        addMarket(address(mockERC4626Market));
        assertEq(etherARM.supportedMarkets(address(mockERC4626Market)), true, "supported pre");

        vm.expectEmit(address(etherARM));
        emit AbstractARM.MarketRemoved(address(mockERC4626Market));

        vm.prank(governor);
        etherARM.removeMarket(address(mockERC4626Market));

        assertEq(etherARM.supportedMarkets(address(mockERC4626Market)), false, "supported post");
    }
    ...
    function test_RemoveMarket_RevertWhen_AddressZero() public {
        vm.prank(governor);
        vm.expectRevert(AbstractARM.InvalidMarket.selector);
        etherARM.removeMarket(address(0));
    }
    ...
    function test_SetActiveMarket_NoPreviousMarket() public {
        addMarket(address(mockERC4626Market));
        assertEq(etherARM.activeMarket(), address(0), "activeMarket pre");
        ...
        assertEq(etherARM.activeMarket(), address(mockERC4626Market), "activeMarket post");
    }

    function test_SetActiveMarket_ToZero() public {
        addMarket(address(mockERC4626Market));
        setActiveMarket(address(mockERC4626Market));
        assertEq(etherARM.activeMarket(), address(mockERC4626Market), "activeMarket pre");

        vm.expectEmit(address(etherARM));
        emit AbstractARM.ActiveMarketUpdated(address(0));

        vm.prank(governor);
        etherARM.setActiveMarket(address(0));

        assertEq(etherARM.activeMarket(), address(0), "activeMarket post");
    }
    ...
    function test_SetActiveMarket_WithPreviousMarket_Empty() public {
        ...
        assertEq(mockERC4626Market.balanceOf(address(etherARM)), 0, "prev market shares pre");

        vm.expectEmit(address(etherARM));
        emit AbstractARM.ActiveMarketUpdated(address(mockERC4626Market2));

        vm.prank(governor);
        etherARM.setActiveMarket(address(mockERC4626Market2));

        assertEq(etherARM.activeMarket(), address(mockERC4626Market2), "activeMarket post");
        assertEq(mockERC4626Market.balanceOf(address(etherARM)), 0, "prev market shares post");
    }
    ...
    function test_SetActiveMarket_WithPreviousMarket_NonEmpty_WithShares() public {
        ...
        uint256 prevMarketShares = mockERC4626Market.balanceOf(address(etherARM));
    ...
    function test_SetActiveMarket_ToSameMarket() public {
        ...
        assertEq(etherARM.activeMarket(), address(mockERC4626Market), "activeMarket unchanged");
    }
    ...
    function test_SetARMBuffer_Default() public {
        ...
        assertEq(etherARM.armBuffer(), 0, "armBuffer pre");
        ...
        assertEq(etherARM.armBuffer(), newBuffer, "armBuffer post");
    }

    function test_SetARMBuffer_RevertWhen_NotAuthorized() public {
        vm.prank(alice);
        vm.expectRevert(OwnableOperable.OnlyOperatorOrOwner.selector);
        etherARM.setARMBuffer(0);
    }
