...
contract Unit_EtherARM_Allocate_Test is Unit_EtherARM_Shared_Test {
    ...
    function test_Allocate_NoOp_WhenAvailableAssetsZero() public {
        ...
        deal(address(weth), address(etherARM), 0);
        addMarket(address(mockERC4626Market));
        setActiveMarket(address(mockERC4626Market));

        assertEq(mockERC4626Market.balanceOf(address(etherARM)), 0, "market shares pre");

        vm.prank(alice);
        etherARM.allocate();

        assertEq(mockERC4626Market.balanceOf(address(etherARM)), 0, "market shares post");
    }
    ...
    function test_Allocate_PositiveDelta_NoOutstandingWithdraw() public {
        ...
        setARMBuffer(1e18);
        setActiveMarket(address(mockERC4626Market));
        assertEq(mockERC4626Market.balanceOf(address(etherARM)), 0, "market pre");
        ...
        assertEq(mockERC4626Market.balanceOf(address(etherARM)), 100 ether + 1e12, "market post");
        assertEq(weth.balanceOf(address(etherARM)), 0, "ARM WETH post");
        assertEq(etherARM.totalAssets(), 100 ether + 1e12, "totalAssets preserved");
    }
    ...
    function test_Allocate_PositiveDelta_WithOutstandingWithdraw() public {
        ...
        assertEq(mockERC4626Market.balanceOf(address(etherARM)), 50 ether + 1e12, "market post");
        assertEq(weth.balanceOf(address(etherARM)), 50 ether, "reserved liquidity stays in ARM");
        assertEq(etherARM.reservedWithdrawLiquidity(), 50 ether, "reservedWithdrawLiquidity");
    }
    ...
    function test_Allocate_NegativeDelta_PartialWithdraw_EnoughLiquidityOnMarket() public {
        ...
        assertEq(mockERC4626Market.balanceOf(address(etherARM)), 100 ether + 1e12, "market pre");
        assertEq(weth.balanceOf(address(etherARM)), 0, "ARM WETH pre");
        ...
        assertEq(weth.balanceOf(address(etherARM)), expectedArm, "ARM WETH post");
        assertEq(mockERC4626Market.balanceOf(address(etherARM)), expectedMarket, "market post");
    }
    ...
    function test_Allocate_NegativeDelta_FullWithdraw_EnoughLiquidityOnMarket() public {
        ...
        assertEq(mockERC4626Market.balanceOf(address(etherARM)), 0, "market post");
        assertEq(weth.balanceOf(address(etherARM)), 100 ether + 1e12, "ARM WETH post");
    }
    ...
    function test_Allocate_NegativeDelta_FullWithdraw_NotEnoughLiquidityOnMarket_AboveThreshold() public {
        ...
        deal(address(steth), address(etherARM), 100 ether);
        ...
        assertEq(mockERC4626Market.balanceOf(address(etherARM)), 0, "market shares post");
        assertEq(weth.balanceOf(address(etherARM)), halved, "ARM WETH post");
        assertEq(steth.balanceOf(address(etherARM)), 100 ether, "ARM stETH unchanged");
    }
    ...
    function test_Allocate_NegativeDelta_FullWithdraw_NotEnoughLiquidityOnMarket_BelowThreshold() public {
        ...
        mockERC4626Market.deposit(MIN_SHARES_TO_REDEEM, address(etherARM));
        vm.stopPrank();
        assertEq(mockERC4626Market.balanceOf(address(etherARM)), MIN_SHARES_TO_REDEEM, "seeded shares");
        ...
        assertEq(mockERC4626Market.balanceOf(address(etherARM)), MIN_SHARES_TO_REDEEM, "market shares unchanged");
        assertEq(weth.balanceOf(address(etherARM)), armWethBefore, "ARM WETH unchanged");
        assertEq(steth.balanceOf(address(etherARM)), 100 ether, "ARM stETH unchanged");
    }
    ...
    function test_Allocate_NullDelta() public {
        ...
        uint256 expectedMarket = (100 ether + 1e12) - expectedArm;
        assertEq(weth.balanceOf(address(etherARM)), expectedArm, "ARM WETH pre");
        assertEq(mockERC4626Market.balanceOf(address(etherARM)), expectedMarket, "market pre");
