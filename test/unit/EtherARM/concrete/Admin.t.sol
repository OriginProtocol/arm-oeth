...
/// @notice Unit tests for the EtherARM admin surface — every owner / operator setter,
...
contract Unit_EtherARM_Admin_Test is Unit_EtherARM_Shared_Test {
    ...
    function _accrueFeesViaSwap() internal returns (uint256 accrued) {
        ...
        vm.prank(alice);
        etherARM.swapTokensForExactTokens(steth, weth, amountOut, expectedAmountIn, alice);

        accrued = etherARM.feesAccrued();
    ...
    function test_AddBaseAsset_Default() public {
        ...
        (,,,,,,,, address adapterBefore) = etherARM.baseAssetConfigs(address(steth));
        assertEq(adapterBefore, address(0), "adapter unset pre");
        assertEq(steth.allowance(address(etherARM), address(stETHAssetAdapter)), 0, "no allowance pre");
        ...
        (
            ...
            uint8 baseDec,
            address adapter
        ) = etherARM.baseAssetConfigs(address(steth));
        ...
        assertEq(
            steth.allowance(address(etherARM), address(stETHAssetAdapter)),
            type(uint256).max,
    ...
    function test_GetBaseAssets_EmptyByDefault() public view {
        ...
        address[] memory assets = etherARM.getBaseAssets();
    ...
    function test_SetPrices_Owner() public {
        ...
        vm.prank(governor);
        etherARM.setPrices(address(steth), newBuy, newSell, newBuyLiq, newSellLiq);
    ...
    function test_SetPrices_Operator() public {
        ...
        vm.prank(operator);
        etherARM.setPrices(address(steth), 0.994e36, 1.003e36, 1 ether, 2 ether);
    ...
    function test_SetPrices_RevertWhen_NotAuthorized() public {
        ...
        etherARM.setPrices(address(steth), BUY_PRICE_DEFAULT, SELL_PRICE_DEFAULT, 1 ether, 1 ether);
    }
    ...
    function test_SetPrices_RevertWhen_SellBelowCross() public {
        ...
        vm.prank(governor);
        vm.expectRevert(AbstractARM.SellPriceTooLow.selector);
        etherARM.setPrices(address(steth), BUY_PRICE_DEFAULT, CROSS_PRICE_DEFAULT - 1, 1 ether, 1 ether);
    }
    ...
    function test_SetPrices_RevertWhen_BuyBelowMinimum() public {
        ...
        vm.prank(governor);
        vm.expectRevert(AbstractARM.InvalidBuyPrice.selector);
        etherARM.setPrices(address(steth), MAX_CROSS_PRICE_DEVIATION - 1, SELL_PRICE_DEFAULT, 1 ether, 1 ether);
    }
    ...
    function test_SetPrices_RevertWhen_BuyAtOrAboveCross() public {
        ...
        vm.prank(governor);
        vm.expectRevert(AbstractARM.InvalidBuyPrice.selector);
        etherARM.setPrices(address(steth), CROSS_PRICE_DEFAULT, SELL_PRICE_DEFAULT, 1 ether, 1 ether);
    }
    ...
    function test_SetCrossPrice_Lower_WithoutExposure() public {
        ...
        vm.prank(governor);
        etherARM.setCrossPrice(address(steth), newCross);
    ...
    function test_SetCrossPrice_Raise() public {
        ...
        vm.prank(governor);
        etherARM.setCrossPrice(address(steth), 0.999e36);

        vm.prank(governor);
        etherARM.setCrossPrice(address(steth), PRICE_SCALE);
    ...
    function test_SetCrossPrice_RevertWhen_UnsupportedAsset() public {
        vm.prank(governor);
        vm.expectRevert(AbstractARM.UnsupportedAsset.selector);
        etherARM.setCrossPrice(address(steth), CROSS_PRICE_DEFAULT);
    }
    ...
    function test_SetCrossPrice_RevertWhen_TooLow() public {
        ...
        vm.prank(governor);
        vm.expectRevert(AbstractARM.CrossPriceTooLow.selector);
        etherARM.setCrossPrice(address(steth), PRICE_SCALE - MAX_CROSS_PRICE_DEVIATION - 1);
    }
    ...
    function test_SetCrossPrice_RevertWhen_TooHigh() public {
        ...
        vm.prank(governor);
        vm.expectRevert(AbstractARM.CrossPriceTooHigh.selector);
        etherARM.setCrossPrice(address(steth), PRICE_SCALE + 1);
    }
    ...
    function test_SetCrossPrice_RevertWhen_SellBelowNewCross() public {
        ...
        vm.prank(governor);
        etherARM.setCrossPrice(address(steth), PRICE_SCALE - MAX_CROSS_PRICE_DEVIATION);
        // Step 2: bring sell to a value below PRICE_SCALE (within the new cross floor).
        vm.prank(governor);
        etherARM.setPrices(address(steth), BUY_PRICE_DEFAULT, PRICE_SCALE - MAX_CROSS_PRICE_DEVIATION, 1 ether, 1 ether);
    ...
    function test_SetCrossPrice_RevertWhen_BuyAtOrAboveNewCross() public {
        ...
        vm.prank(governor);
        etherARM.setPrices(address(steth), 0.999e36, SELL_PRICE_DEFAULT, 1 ether, 1 ether);
    ...
    function test_SetCrossPrice_RevertWhen_TooManyBaseAssets() public {
        ...
        deal(address(steth), address(etherARM), 1 ether);
    ...
    function test_SetFee_Default() public {
        ...
        vm.prank(governor);
        etherARM.setFee(newFee);

        assertEq(etherARM.fee(), newFee, "fee updated");
    }
    ...
    function test_SetFee_FlushesAccruedFees() public {
        ...
        assertEq(etherARM.fee(), 500, "fee updated after flush");
    }
    ...
    function test_SetFeeCollector_Default() public {
        ...
        vm.prank(governor);
        etherARM.setFeeCollector(newCollector);

        assertEq(etherARM.feeCollector(), newCollector, "feeCollector updated");
    }

    function test_SetFeeCollector_RevertWhen_NotOwner() public {
        vm.prank(alice);
        vm.expectRevert(Ownable.OnlyOwner.selector);
        etherARM.setFeeCollector(makeAddr("rejected"));
    }

    function test_SetFeeCollector_RevertWhen_ZeroAddress() public {
        vm.prank(governor);
        vm.expectRevert(AbstractARM.InvalidFeeCollector.selector);
        etherARM.setFeeCollector(address(0));
    }
    ...
    function test_CollectFees_ZeroAccrued_ReturnsZeroNoTransfer() public {
        assertEq(etherARM.feesAccrued(), 0, "feesAccrued starts at 0");
        uint256 collectorBefore = weth.balanceOf(feeCollector);

        uint256 collected = etherARM.collectFees();
    ...
    function test_CollectFees_RevertWhen_InsufficientLiquidity() public {
        ...
        vm.prank(alice);
        etherARM.requestRedeem(95 ether);
    ...
    function test_SetCapManager_ToNonZero() public {
        ...
        vm.prank(governor);
        etherARM.setCapManager(newCapManager);

        assertEq(etherARM.capManager(), newCapManager, "capManager updated");
    }
    ...
    function test_SetCapManager_ToZero_DisablesCaps() public {
        ...
        assertEq(etherARM.capManager(), address(0), "capManager cleared");
    }

    function test_SetCapManager_RevertWhen_NotOwner() public {
        vm.prank(alice);
        vm.expectRevert(Ownable.OnlyOwner.selector);
        etherARM.setCapManager(makeAddr("rejected"));
    }
    ...
    function test_SetARMBuffer_Owner() public {
        ...
        vm.prank(governor);
        etherARM.setARMBuffer(newBuffer);

        assertEq(etherARM.armBuffer(), newBuffer, "armBuffer updated by owner");
    }

    function test_SetARMBuffer_Operator() public {
        vm.prank(operator);
        etherARM.setARMBuffer(0.5e18);
        assertEq(etherARM.armBuffer(), 0.5e18, "armBuffer updated by operator");
    }

    function test_SetARMBuffer_BoundaryMax() public {
        // 100% is the inclusive upper bound (1e18).
        vm.prank(governor);
        etherARM.setARMBuffer(1e18);
        assertEq(etherARM.armBuffer(), 1e18, "armBuffer at boundary 1e18");
    }

    function test_SetARMBuffer_RevertWhen_NotAuthorized() public {
        vm.prank(alice);
        vm.expectRevert(OwnableOperable.OnlyOperatorOrOwner.selector);
        etherARM.setARMBuffer(0.1e18);
    }
