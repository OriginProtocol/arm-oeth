...
contract Fork_Concrete_LidoARM_Setters_Test_ is Fork_Shared_Test_ {
    ...
    function test_RevertWhen_SetPrices_Because_PriceRange_Operator() public asOperator {
        ...
        lidoARM.setPrices(address(steth), 1.0001 * 1e36, 1.002 * 1e36, type(uint256).max, type(uint256).max);

        // sell price 11 basis points lower than 1.0
        vm.expectRevert("ARM: sell price too low");
        lidoARM.setPrices(address(steth), 0.998 * 1e36, 0.9989 * 1e36, type(uint256).max, type(uint256).max);

        // Forgot to scale up to 36 decimals
        vm.expectRevert("ARM: sell price too low");
        lidoARM.setPrices(address(steth), 1e18, 1e18, type(uint256).max, type(uint256).max);
    }
    ...
    function test_RevertWhen_SetPrices_Because_NotOwnerOrOperator() public asRandomAddress {
        ...
        lidoARM.setPrices(address(steth), 0, 0, type(uint256).max, type(uint256).max);
    }
    ...
    function test_RevertWhen_SetPrices_Because_BuyPriceCannotCrossOneByMoreThanTenBps() public asOperator {
        vm.expectRevert("ARM: buy price too high");
        lidoARM.setPrices(address(steth), 1.0011 * 1e36, 1.002 * 1e36, type(uint256).max, type(uint256).max);
    }
    ...
    function test_SetPrices_Operator() public asOperator {
        ...
        lidoARM.setPrices(address(steth), 9980e32, 99998e32, type(uint256).max, type(uint256).max);
        // 2% of one basis point spread
        lidoARM.setPrices(address(steth), 999999e30, 1000001e30, type(uint256).max, type(uint256).max);

        lidoARM.setPrices(address(steth), 992 * 1e33, 1001 * 1e33, type(uint256).max, type(uint256).max);
        lidoARM.setPrices(address(steth), 99999e31, 1004 * 1e33, type(uint256).max, type(uint256).max);
        lidoARM.setPrices(address(steth), 992 * 1e33, 2000 * 1e33, type(uint256).max, type(uint256).max);
