...
contract Fork_Concrete_LidoARM_SetCrossPrice_Test_ is Fork_Shared_Test_ {
    ...
    function test_RevertWhen_SetCrossPrice_Because_BuyPriceTooHigh() public {
        lidoARM.setPrices(address(steth), 1e36 - 20e32 + 1, 1000 * 1e33 + 1);
    ...
    function test_RevertWhen_SetCrossPrice_Because_SellPriceTooLow() public {
        ...
        lidoARM.setPrices(address(steth), 1e36 - 20e32, 1000 * 1e33 + 1);
        // This allow us to set a cross price below 1e36
        lidoARM.setCrossPrice(address(steth), 1e36 - 20e32 + 1);
        // Then we make both buy and sell price below the 1e36
        lidoARM.setPrices(address(steth), 1e36 - 20e32, 1e36 - 20e32 + 1);
    ...
    function test_SetCrossPrice_No_StETH_Owner() public {
        ...
        emit AbstractMultiAssetARM.CrossPriceUpdated(address(steth), 1e36);
        lidoARM.setCrossPrice(address(steth), 1e36);

        // 20 basis points lower than 1.0
        vm.expectEmit({emitter: address(lidoARM)});
        emit AbstractMultiAssetARM.CrossPriceUpdated(address(steth), 0.998e36);
        lidoARM.setCrossPrice(address(steth), 0.998e36);
    }
    ...
    function test_SetCrossPrice_With_StETH_PriceUp_Owner() public {
        ...
        lidoARM.setCrossPrice(address(steth), 0.9999e36);
    }
}
