...
const estimateAverageWithdrawTime = async (
  ...
  log(
    `Vault Outstanding amount: ${formatUnits(vaultQueuedWithdrawals[0] - vaultQueuedWithdrawals[2], 18)}`,
  );
  ...
  const oSAvailable =
    armOutstandingWithdrawals > oSBalanceInARM
      ? 0n
      : oSBalanceInARM - armOutstandingWithdrawals;
  ...
  if (wSAvailable >= oSAvailable) {
    log(
      `More wS available in Vault than OS in ARM, withdrawal time estimated to be close from: 1 day\n`,
    );
    return 86400n;
  }
  ...
  log(
    `Amount to fetch from undelegating requests : ${formatUnits(amount, 18)}\n`,
  );
  ...
  log(
    `Total request withdrawals                  : ${totalWithdrawalsCount[3]}`,
  );
  ...
  const recentWithdrawalRequests = await Promise.all(
    Array.from(
      {
        length: Math.min(
          numberOfRecentRequests,
          Number(totalWithdrawalsCount[3]),
        ),
      },
      (_, i) =>
        vault.withdrawalRequests(Number(totalWithdrawalsCount[3]) - 1 - i, {
          blockTag,
        }),
    ),
  );
  ...
  log(
    `Unclaimed request count                    : ${unclaimedRequests.length.toString()}`,
  );

  // Filter request that are older than 13 days
  const oldRequests = unclaimedRequests.filter(
    (req) => BigInt(timestamp) - req[2] >= 13 * 86400,
  );
  log(
    `Unclaimed request > 13 days old            : ${oldRequests.length.toString()}`,
  );
  ...
  log(
    `Total amount from request > 13 days old    : ${formatUnits(sumOfOldRequestAmount, 18)}`,
  );
  ...
  log(
    `Amount remaining minus > 13 days old reques: ${formatUnits(amount, 18)}`,
  );

  // Filter request that are younger than 13 days (and not claimed)
  const recentRequests = unclaimedRequests.filter(
    (req) => BigInt(timestamp) - req[2] < 13 * 86400,
  );
  log(
    `Unclaimed request < 13 days old            : ${recentRequests.length.toString()}`,
  );
  ...
  log(
    `Total Amount (include all requests)        : ${formatUnits(totalAmount, 18)}\n`,
  );
